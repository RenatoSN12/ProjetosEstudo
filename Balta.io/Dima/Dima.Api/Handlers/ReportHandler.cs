...
public class ReportHandler(AppDbContext context) : IReportHandler
{
        ...
        GetIncomesAndExpensesRequest request)
    {
        ...
        {
            return new Response<List<IncomesAndExpenses>?>(null, code: 500,
                message: "Não foi possível obter as entradas e saídas.");
        }
    }
        ...
        GetExpensesByCategoryRequest request)
    {
        ...
        catch
        {
            return new Response<List<ExpensesByCategory>?>(null, 500,
                "Não foi possível obter as saídas por categoria.");
        }
    }
        ...
        try
        {
            ...
            var data = await context
                ...
                .Select(x => new FinancialSummary(
                    request.UserId,
                    x.Where(ty => ty.Type == ETransactionType.Deposit).Sum(am => am.Amount),
                    x.Where(ty => ty.Type == ETransactionType.Withdraw).Sum(am => am.Amount)))
                .FirstOrDefaultAsync();
